...
namespace lve
{
    RenderSetup setupRender(LveDevice &lveDevice) {
        ...
        for (int i = 0; i < setup.uboBuffers.size(); i++) {
            setup.uboBuffers[i] = std::make_unique<LveBuffer>(lveDevice, sizeof(GlobalUbo), 1, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
            setup.uboBuffers[i]->map();
        }

        setup.globalSetLayout = LveDescriptorSetLayout::Builder(lveDevice)
        ...
        for (int i = 0; i < setup.globalDescriptorSets.size(); i++) {
            ...
            LveDescriptorWriter(*setup.globalSetLayout, *setup.globalPool).writeBuffer(0, &bufferInfo).writeImage(1, &imageInfo).build(setup.globalDescriptorSets[i]);
        }
