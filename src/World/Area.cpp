...
namespace lve
{
    ...
    Area::Area(LveDevice &lveDevice, s64 seed, Coordinator &coordinator, ChunkStagingPool &chunkMeshPool)
        : device(lveDevice), chunkGenSystem(*this), chunkMeshSystem(*this, device, chunkMeshPool), chunkMutationSystem(), worldSeed(seed), coordinator(coordinator), chunkMeshPool(chunkMeshPool)
    {
        std::cout << "seed " << seed << '\n';
    }
    ...
    bool Area::isBlockSolid(glm::vec3 worldBlockPos, glm::vec3 rayPos, glm::vec3 rayDirection)
    {
        ...
        const Block *voxel = optionalVoxel;
